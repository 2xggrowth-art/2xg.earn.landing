  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        >
          A <span className="text-emerald-600">90-day operating rhythm</span>{" "}
        ...
        <ol className="relative mt-10 flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:pb-0">
          ...
          {steps.map((step, index) => (
            <li
              key={step.title}
              className="relative flex min-w-[85vw] flex-shrink-0 flex-col gap-4 rounded-2xl border border-slate-200 bg-white/60 p-5 shadow-sm snap-center md:min-w-0 md:w-auto"
            >
              ...
              <p className="text-xs text-slate-900 sm:text-sm">{step.description}</p>
              <ul className="mt-2 space-y-2 text-xs text-slate-900 sm:text-sm">
                {step.points.map((p) => (
